...
A closed product process id accepted by the tool schema. The active diagnostic ids are `morning_planning`, `midday_adjustment`, `personal_context_review`, `consent_and_privacy`, `evening_reflection`, `weekly_summary`, and `final_report`; retired `morning_activity_collection` and disabled `day_focus` are rejected.

...
| `/bin/collect-activities.md` | `collectActivities` | Yes, canonical structured write | Employee and tenant ids bound by `AssistantService` | Record every explicitly named completed or in-progress activity as a separate structured item in one batch; omit unknown values and accept no free text. |
| `/bin/read-recent-own-activities.md` | `readRecentOwnActivities` | No | Authenticated employee, company, and group | Return at most five active own activities from the last 72 hours for an explicit correction lookup; closed facets plus opaque handle/revision, without identity fields or raw text. |
| `/bin/correct-recent-activity.md` | `correctRecentActivity` | Yes, revisioned local repair | Authenticated employee, company, and group; exact recent handle/revision | Patch or replace closed facets on one canonical row while retaining evidence and revision provenance. |
| `/bin/supersede-recent-activity.md` | `supersedeRecentActivity` | Yes, revisioned local repair | Authenticated employee, company, and group; exact recent handle pair/revisions | Exclude one confirmed duplicate from current projections while preserving it and its replacement link for research. |

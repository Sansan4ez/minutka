...
| `/bin/propose-task-mutation.md` | `proposeTaskMutation` | Depends on current typed result during rollout | Level 0: create/update/complete; level 1: cancel | Owner bound by `AssistantService`; task id is application-generated and private | Prepare or apply one create/update/complete/cancel operation per turn. |
| `/bin/propose-idea-to-task.md` | `proposeIdeaToTask` | Depends on current typed result during rollout | Level 0 policy; typed result remains authoritative during rollout | Owner and idea/task provenance bound privately by application use-case | Prepare or apply one idempotent idea-to-task conversion per turn. |

...
Read, list, and search are tools because they are deterministic typed operations over an owner-scoped namespace. `listProjects` exposes the existing string labels across ideas and tasks; it does not create a project entity. `inbox_capture` is a business process because the agent must interpret the inbound item, choose its project and record type, decide whether clarification is needed, and then invoke `captureIdea`.

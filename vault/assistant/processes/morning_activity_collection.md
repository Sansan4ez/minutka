...
3. Call `collectActivities` once with one array item for each named activity.
...
A concise Telegram-friendly invitation or acknowledgement. For an employee account, produce one successful `collectActivities` call containing every named activity.

...
5. For reminders: the bot sends one predefined soft message itself when touches stop, at most once per 24 hours and a bounded number of times; there is no operator broadcast command. Then the methodologist may contact the employee and later tell the company lead only the participation fact. Conversation content is not used; only this first tier is automatic.
...
- A reminder answer with the automatic first tier, its limits, and three tiers.
...
- Selecting reminders from content or inferred judgements; hiding the reminder limits; calling a live tier automatic.

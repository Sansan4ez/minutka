...
5. Send closed values only. System/obstacle facets: unnamed or unknown → omit; covered → enum; explicitly known but uncovered → `other` where available. A meeting or call without a named channel has no system. Never pass free text to `collectActivities`.
6. Put category and at most one obstacle in one item. Energy/stress has no `other` and requires an explicit covered signal. Ordinary meeting, call, work, or template preparation without named friction has no obstacle. Do not infer emotion.

...
2. For fresh completed or in-progress work, call `processCurrentActivityTurn({ mode: "record" })` once. Repeated real work is a new factual episode. Use `record`; duration-only reply, always uses `repair` and creates no new activity.

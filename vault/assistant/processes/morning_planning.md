...
3. Never call `collectActivity` for a plan, intention, future task, or work that has not started. Morning planning is read-only.
...
5. Only after the employee explicitly names yesterday's factual completed or in-progress activities, call `collectActivity` exactly once per named activity, up to three. Never record planned or not-started work. Omit unknown closed fields and never pass free text.

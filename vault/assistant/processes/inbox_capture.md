...
4. No clear match: create immediately. One clear match: ask one plain-text question before writing: “Похоже на запись от 21:05 про бассейн — дополнить её или завести отдельную?” Supplement via `appendIdea` (дополнение не откатывается)/task update; otherwise create separately. Silence/ambiguity means create separately. A possible duplicate is cheaper than dropped input.

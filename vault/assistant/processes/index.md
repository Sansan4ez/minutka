...
| `evening_reflection` | End-of-day facts, blockers, work-related energy, or a scheduled evening trigger. | For a fresh factual account call `processCurrentActivityTurn` once with `record`, or with `repair` only for an explicit correction/confirmed duplicate. After daytime writes, ask what else to add to what is already noted. Never record planned/not-started work; then respond from the typed transaction result with a concise non-judgmental reflection and at most one next step. |

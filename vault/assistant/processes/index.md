...
- For task requests, use `listTasks` as needed and prepare at most one create/update/complete/cancel with `proposeTaskMutation` (or one idea provenance proposal with `proposeIdeaToTask`). For “mark X completed”, explicitly call `listTasks` to resolve the current task id and revision, then call `proposeTaskMutation({ kind: "complete", taskId, expectedRevision })` and show the resulting proposal to the owner. The application exposes the safe pending action; authenticated confirm/reject commands run outside the agent tool loop. Never claim mutation from a proposal alone.

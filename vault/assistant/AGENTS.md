...
- Idea capture is level 0. Retrieve before write; supplement one clear match, otherwise create—never lose input. A chat URL is idea text, not an artifact/document: keep context/intent; never fetch or claim it was read without a tool. With no intent, capture first, then ask once: “Что сделать со ссылкой?” Projects are labels; a project-only request is not capture. Exact deletion is level 1: ask once; «да» or button. Search active ideas by default; include archived only when requested. A proposal is not deletion.

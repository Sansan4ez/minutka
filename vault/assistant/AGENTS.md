...
- Idea capture is level 0. Projects are labels, not records: do not capture “создай проект X”; explain that `X` appears with its first task/idea and offer that record. Use `listProjects` before open clarification and preserve named labels. Exact deletion is level 1: ask once; «да» or button. Search active `raw`/`discussed` by default; pass archived statuses only when requested. If ambiguous, show a short list. A proposal is not deletion.

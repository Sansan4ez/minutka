...
| 7 | `/proc/context` | Приоритетные личные документы | untrusted owner data, current `userId` | chat и будущие scheduled jobs | 12 docs; 4 000/doc; 16 000 total | включён в chat |
...
- Owner projections экранируются и маркируются как untrusted data. Их scope формирует application layer из authenticated `userId`/`threadId`.

...
- Core-документы, совпавшие с trusted priority manifest, валидируются против per-file, count и суммарного context ceiling при discover/import пилотной БЗ; невалидные данные отклоняются до записи в store. Runtime сохраняет тот же throw как defense-in-depth. Если прочий owner document не помещается целиком, runtime оставляет явный truncation/index-reference с логическим path, исходным размером и способом продолжить через `readDocument`. Audit содержит только source id, размеры, counts и причины, без текста owner documents.
...
- `/proc/thread` двухслойный: сначала `thread_summary` (≤4 000 chars), затем 10 свежих completed turns дословно (≤12 000 chars). После durable append ответа асинхронная job детерминированно выбирает turns вне recent-window. Она сначала запускает существующий insight extractor для выпавших turns, затем summarizer; сбои этих шагов независимы и фиксируются metadata-only audit. Summarizer получает только предыдущий checkpoint + новые выпавшие turns и не имеет write-портов. Watermark — inclusive диапазон `fromMessageId..throughMessageId`; повторная job читает только turns после `throughMessageId`, поэтому идемпотентна. При превышении ceiling выполняется отдельный reduction pass с явной строкой о сокращении; byte/character slice итогового текста запрещён. При сбое остаётся предыдущий валидный checkpoint и существующий history truncation marker. `ConversationStore` и `DocumentStore` не изменяются компакшном: raw turns остаются каноническими и summary можно регенерировать по watermark.

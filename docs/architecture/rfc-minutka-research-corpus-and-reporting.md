...
**Должность — точный справочник, а не инструмент приватности.** `roles` заводятся per-company по фактической функции: два логиста делят один `logistician`, два менеджера отдела продаж — один `sales_manager`; персональные и искусственно укрупнённые роли («менеджер», «коммерческий блок») запрещены. Укрупнение ради порога свидетельств смешало бы разные процессы и обесценило карту автоматизации.

**Reporting scope — отдельная политика приватности.** Достаточность свидетельств выражается confidence и `insufficient_data` (§2.9), а не составом справочника должностей. Если `roleId` рутины существует в per-company справочнике `roles`, роль всегда становится scope этой рутины в клиентском отчёте независимо от числа contributors: это функциональная привязка работы, необходимая карте автоматизации, а не оценка сотрудника. Отдельная ось `department` в первой когорте не вводится: подразделения этой группы не набирают privacy-safe размер, а искусственное объединение смешало бы процессы.

Словарь систем устроен обратным образом: он глобальный и закрытый, описывает *типы* систем, а бренды и внутренние названия компании маппятся на generic-значения до старта сбора — см. [runbook справочников](../runbooks/tenant-reference-directories.md#generic-mapping-типов-систем).
...
Report показывает confidence и ограничения. Конкретная рекомендация по роли с одним contributor допустима как гипотеза, если она описывает процесс/автоматизацию и не превращается в оценку единственного сотрудника. Роль остаётся scope рутины, а содержание карточки ограничивается: приблизительные часы как порядок величины по самоотчёту и факты трения остаются, сигналы энергии/стресса о состоянии человека не показываются. `coverage.limitations` для каждой такой роли содержит строку «Роль <label> представлена одним участником; её рутины — самоотчёт одного человека, не оценка».

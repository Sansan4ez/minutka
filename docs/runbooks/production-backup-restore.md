...
- `minutka.dump` содержит переписку, профили и согласия всех owner'ов
  **открытым текстом**; доступ к копии равен доступу к персональным данным
  пилота;
- integration credentials и chat id в дампе лежат **зашифрованными**
  (`INTEGRATION_ENC_KEY`), поэтому копия сама по себе не раскрывает токены
...
враждебны. Шифрование backup'ов (`age`, `restic`, `borg`) добавляет ключ,
который сам нужно хранить, ротировать и не потерять — при потере ключа копия
превращается в мусор ровно тогда, когда она нужна. Для пилота этот риск выше
защищаемого.

**Пересмотреть вместе с планкой** при любом триггере §7 RFC: выход за
доверенный круг, первый платящий клиент, импорт данных третьих лиц. Шифрование
копий входит в тот же пакет, что и полноценный security-аудит, а не
внедряется отдельно.

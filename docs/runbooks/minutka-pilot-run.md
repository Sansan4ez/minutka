...
Известное расхождение на 2026-08-17: ответ сохраняется как «идея» унаследованного ассистента (`captureIdea`), обе таблицы остаются пустыми — `mnt-pilot-readiness-w73.10`. До его закрытия шаг отмечается `нет`.

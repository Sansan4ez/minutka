...
**Признак `прошло`:** дневной turn получает непустой текстовый ответ; пять factual activities дают пять новых canonical строк с одним subject binding сотрудника, `source_message_id` текущего turn, локальной `activity_date` и точным company/group scope. Будущее намерение про презентацию строки не создаёт. В последнем trace `attempts[].contour` различает `main_agent` и `activity_transaction`: main agent вызывает только `processCurrentActivityTurn`, а transaction attempt содержит anchor `minutka-activity-transaction/v1`, sanitized bounded context, strict decision `collect` с пятью evidenced patches, typed mutation result с `savedCount = 5`, latency и отдельный usage. `activity_transaction.usage.llmSteps = 1`; длина его `context` существенно меньше `main_agent.context`. Conversation turn и full research trace сохранены, research export выбранной группы связывает messages, activities и traces по subject/evidence refs. Отдельной reporting-копии activity нет, а client report не содержит transaction context/decision/subject refs.

Focused live gate после ordinary сообщения повторяет на том же сотруднике: явные combined facets (task/routine/automation/energy/system/duration), повтор той же реальной работы как новую строку, русскую correction, Spanish duplicate confirmation и ambiguous repair. Ordinary combined-facet turn вызывает только `processCurrentActivityTurn`: `typical_tasks`, другие confirmed profile fields и число `profile_updated` audit-событий остаются без изменений. Correction и duplicate дают по одной bounded mutation; ambiguity не пишет. Затем второй employee/company scope подтверждает isolation, а client artifact повторно проверяется на отсутствие `subjectKey`, trace attempts, extractor context и decision. Для before/after evidence сохранить из одинаковых ordinary/correction/duplicate traces: provider calls, total/input/cached tokens, main/transaction context chars и latency. Gate не задаёт абсолютный token target: after не увеличивает число provider round trips текущего успешного agentic flow, transaction generation остаётся одним вызовом, а extraction context materially smaller broad main context.
...
Финальный личный отчёт цикла default-расписания не имеет: в конце двух недель его ставит оператор на всю группу командой `npm run cycle:final-reports` — порядок и проверки в [runbook завершения цикла](end-of-cycle.md).

...
**Baseline снят по колонке, которая содержит токены последнего LLM-шага, а не всего хода, и подлежит пересчёту после `prs-ip0.10`.** `src/mastra/agent-runner.ts` берёт `result.usage`, а Mastra документирует это поле как usage последнего шага (`totalUsage` — сумма по всем шагам). При `maxSteps: 4` ход с tool-loop попадает в `minutka_private.usage` одним шагом. Цифры выше поэтому направленно верны, но являются нижней границей неизвестного размера; принимать по ним решения о ceiling'ах и ставках до пересчёта нельзя.
...
Текущая monthly сумма остаётся нижней оценкой полного расхода owner contour по двум независимым причинам:

1. вспомогательные LLM-вызовы идут мимо счётчика — `prs-ip0.4`;
2. записанные токены относятся к последнему LLM-шагу хода, а не к сумме шагов — `prs-ip0.10`.

Пока `prs-ip0.10` не закрыта, расхождение видно прямо в operational-логе: ход помечается `llmSteps: 4` и тарифицируется как один шаг. По той же причине cache hit rate по этому срезу считать нельзя — `cachedInputTokens` сейчас суммируется по всем шагам, а `inputTokens` относится к одному, то есть отношение `sum(cachedInputTokens) / sum(inputTokens)` из п.4 процедуры выше даст завышенный результат. Первым шагом перекалибровки должна быть `prs-ip0.10`, вторым — повторный замер.

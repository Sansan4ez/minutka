...
`llmSteps` и `cachedInputTokens` пока operational-only: они сознательно не меняют usage schema до выполнения отдельной задачи `prs-ip0.4`, которая добавит source attribution и учёт всех вспомогательных LLM-вызовов. Следовательно, текущая monthly сумма остаётся нижней оценкой полного расхода owner contour.

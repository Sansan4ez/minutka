...
# privacy-v6 policy snapshot and set PRIVACY_POLICY_V6_URL to its public URL.
```
...
PRIVACY_POLICY_V6_URL=https://privacy.example.com/privacy-v6.html
TELEGRAM_BOT_TOKEN=...
...
`PRIVACY_POLICY_V6_URL` is mandatory deployment configuration, with no repository
fallback. Before accepting employees, publish the exact `privacy-v6` policy snapshot
at an anonymously accessible HTTPS URL and verify it from outside the deployment
network. A normal canonical URL must include `privacy-v6` as a path segment or
filename (for example `/privacy-v6.html`). GitHub and raw GitHub document URLs
...
или до появления недельного касания, запустите идемпотентный backfill с
runtime-учётными данными:
...
Команда выбирает `profile_completed`, у которых нет хотя бы одного дефолтного
расписания, и создаёт только недостающие строки в таймзоне профиля: утро
`08:30` и вечер `19:00` по будням, недельная сводка `17:00` по пятницам.
Повторный запуск безопасен. Уже настроенное или выключенное владельцем
расписание не меняется и не включается заново.

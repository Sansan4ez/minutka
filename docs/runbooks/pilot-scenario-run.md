...
> **Происхождение: унаследован от репозитория персонального ассистента.** Это гейт **чужой фазы** (D.0), а не «Минутки»: он предполагает живого тестировщика в Telegram, два календарных дня и утренний процесс `day_focus`, который в «Минутке» заменён на `morning_activity_collection` ([RFC «Минутки»](../architecture/rfc-minutka-tenancy-and-reporting.md), `src/application/default-schedules.ts`). Документ оставлен как провенанс и не переписывается «поверх». Прогон «Минутки» — [minutka-pilot-run.md](./minutka-pilot-run.md).

Этот чек-лист — integration gate фазы D.0 по [планке качества пилота §2.2 п. 4](../architecture/rfc-pilot-quality-bar.md#22-бюджет-ревью). Прогон не является smoke-spec: он занимает два календарных дня, использует живой Telegram и живой LLM и проверяет реальные доставки по расписанию. Для первого чистого production запуска сначала выполнить [production clean cutover](./production-clean-cutover.md): dev durable storage не переносится, а Telegram polling переключается по single-poller sequence.
...
- сначала выполнить dry-run, затем реальный импорт и повторный идемпотентный запуск; после первого успеха MinIO — единственный source of truth, а отличающаяся Git-копия даёт conflict и не перезаписывает canonical document;
...
`lagging` означает два завершённых локальных будних дня после последнего касания, `dropped_off` — три и более; выходные и текущий незавершённый локальный день не считаются;
...
| 3 | Тестировщик проходит онбординг: выбирает часовой пояс кнопкой из списка (либо «Другой» и пишет свой город, IANA или смещение вида `UTC+3`) и подтверждает анкету; затем открывает переданную оператором [карту навыков](../product/skills-map.md). | Бот подтверждает сохранение профиля и присылает первое приветственное сообщение; тестировщик понимает, какие возможности уже работают, подключаются или только задуманы. Часовой пояс проверяется по времени, а не по написанию идентификатора — см. критерии ниже. | | |
...
### Критерии шага 3: часовой пояс

Пояс определяется выбором и проверяется временем, а не написанием идентификатора. Тестировщику не нужно знать, что такое IANA. Шаг 3 считается `прошло`, только если верны все три пункта:

- бот показывает пояс **кнопками** (Калининград, Москва, Самара, Екатеринбург, Омск, Красноярск, Иркутск, Владивосток, «Другой»), а не просит ввести идентификатор;
- вариант «Другой» принимает город (`Новосибирск`), IANA (`Asia/Novosibirsk`) или смещение (`UTC+7`, `МСК+4`); нераспознанный ввод отвечает «Не узнал этот пояс» и снова показывает кнопки, а не повторяет тот же вопрос молча;
- в сводке подтверждения виден пояс и текущее локальное время в нём — `часовой пояс: Asia/Yekaterinburg (сейчас у вас 14:32)` — и это время совпадает с часами тестировщика.

Третий пункт — основная проверка: расхождение времени означает неверный пояс, и это выяснится здесь, а не через сутки на несработавшем утреннем касании.

...
Это удаляет целиком записи, чья provenance пересекает точную пару, сохраняет незатронутый остаток в новой версии и добавляет удалённые id в tombstones. После этого отчёт пересчитывается по очищенному справочнику; activities других сотрудников не удаляются. Новый доступ создаётся только новым инвайтом.

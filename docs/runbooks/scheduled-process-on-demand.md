...
Ожидаемое содержание: приглашение назвать фактически выполненные или начатые активности, препятствие и необязательный рабочий сигнал энергии — без продуктового лимита количества. После ответа сотрудника все названные фактические activities передаются минимальным числом вызовов `collectActivities` под 50-item batch-границей; планы и не начатая работа не записываются.
...
Ожидаемое содержание: итог по собственным активностям сотрудника за **период учебной группы** (`training_groups.period`, обе даты включительно; если цикл ещё идёт — по сегодняшний локальный день сотрудника, дата запуска выборку не выбирает) — повторяющиеся задачи, места трения и явно названные сигналы энергии — с одним завершающим вопросом о том, какую подтверждённую рутину сотрудник хотел бы упростить. Паттерном называется только то, что повторилось в цикле минимум дважды; отбор делает `readCycleActivities`, а не модель. Ответ не содержит legacy-категорий как видов работы, quick wins, рекомендаций, шаблонов, чек-листов или придуманного плана действий: назначение quick win живёт только в операторском справочнике, а ответ сотрудника на вопрос становится corpus-сигналом для методолога. Если за окно записано меньше шести активностей или они попали меньше чем в четыре дня, ответ прямо говорит о нехватке данных за цикл и не достраивает картину. Процесс read-only: активности не записываются и профиль не меняется. Отчёт личный — в артефакт компании он не попадает. Расписания по умолчанию у него нет; разовое касание на группу ставит оператор — см. [завершение цикла](end-of-cycle.md).
...
При успехе stdout содержит текст ответа процесса, команда завершается кодом `0`. Ответ и вызов сохраняются в личной истории этой ветки обычным application use-case; Telegram-сообщение не отправляется. Поэтому текст личного отчёта репетируется только на изолированном клоне БД, а on-demand запуск `final_report` на production выполняется только для реальной доставки.

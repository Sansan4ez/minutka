...
## Удаление и замена регулярных задач

Список регулярных задач по умолчанию пополняется: обычный рассказ о работе добавляет задачу и не трогает уже сохранённые. Явная просьба убрать или переписать задачу идёт тем же сообщением, например:

```text
Убери из моих регулярных задач подготовку заявок.
```

Агент вызывает тот же owner-bound `updatePersonalContext`, но с `typicalTasksMode: "replace"` и полным исправленным списком: сохранённый список становится ровно тем, что он прислал. Проверка — `/context` до и после: удалённая задача исчезла, остальные на месте. Тот же режим применяется, когда список уже дошёл до предела в семь задач и новая названная задача иначе была бы молча отброшена.

Границы режима:

- замена идёт только по явной просьбе сотрудника; агент не переписывает список по своей инициативе;
- список нельзя опустошить из разговора — в замене остаётся минимум одна задача. Полная очистка сотруднику в пилоте недоступна ни на одной поверхности (HTTP `PATCH /v1/me/context` и CLI тоже требуют непустой список);
- в research trace от вызова остаются только имена полей (`typicalTasks`, `typicalTasksMode`), значения задач в trace не попадают.

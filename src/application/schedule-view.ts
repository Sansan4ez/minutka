...
export function toScheduleView(schedule: ProcessSchedule): ScheduleView {
  if (!(assistantDiagnosticProcessIds as readonly string[]).includes(schedule.processId)) {
    throw new Error(`Unsupported assistant schedule process: ${schedule.processId}`);
  }
  return {
    id: schedule.id,

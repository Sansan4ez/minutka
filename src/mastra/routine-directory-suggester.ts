...
export const buildRoutineDirectorySuggestPrompt = (input: Parameters<RoutineDirectorySuggestionGenerator>[0]): string => [
  ...
  "Attach only to an id in this role section. For create, use a concise routine name and description, choose one work category from the supplied closed list, and choose a quick win from the catalog or deep_dive. Keep proposals as suggestions for a methodologist; do not edit the directory.",
  ...
  `# Work categories\n${JSON.stringify(input.workCategories)}`,

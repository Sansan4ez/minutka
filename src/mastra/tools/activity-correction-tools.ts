...
export function createCorrectRecentActivityTool(
  ...
  return createTool({
    ...
    description: "Correct exactly one recent activity selected from readRecentOwnActivities. Use only after the employee explicitly corrects or clarifies that episode and one candidate is unambiguous. Pass its opaque handle and revision. mode patch changes only supplied closed facets; mode replace clears omitted facets and replaces the closed facet set. Never create a new activity for the correction, never pass identity fields or free text, and never retry a conflict with a guessed revision.",
    strict: true,

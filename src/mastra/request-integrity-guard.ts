...
function buildRequestIntegrityPrompt(text: string): string {
  return [
    ...
    "Allow ordinary owner-scoped search, listing, reading, and mutations. Typed business capabilities independently bind the authenticated owner and enforce confirmation where required.",
    "A request to use a capability is not check evasion or laundering unless it explicitly asks to bypass its owner scope, confirmation, audit, or another authority boundary.",
    "Deny forbidden_action_laundering when the request explicitly targets another owner's data or explicitly asks to mutate or perform an external action without required confirmation.",

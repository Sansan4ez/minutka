    ...
    async listActivities({ companyId, groupId }) {
      try {
        const result = await pool.query<ActivityRow>(
          `SELECT activity_id, subject_key, company_id, group_id, role_id, task_category, obstacle_kind, obstacle_value, duration_bucket, system, recorded_at
           FROM minutka_private.activities WHERE company_id=$1 AND group_id=$2 ORDER BY recorded_at, activity_id`,
          [companyId, groupId],
        );
        return result.rows.map((row) => ({
          activityId: row.activity_id, subjectKey: row.subject_key, companyId: row.company_id, groupId: row.group_id, roleId: row.role_id,
          ...
          ...(row.system ? { system: row.system } : {}), recordedAt: row.recorded_at.toISOString(),
        }));
      } catch (error) { throw mapPostgresError(error); }
    },

...
export async function runMinutkaCli(client: EmployeeMinutkaClient | AdminMinutkaClient, argv: string[], env: NodeJS.ProcessEnv = process.env): Promise<CliResult> {
  ...
  employee.addCommand(new Command("complete-onboarding").option("--name <name>", "How the assistant addresses the employee").option("--assistant-name <assistantName>").option("--address-form <addressForm>", "informal|formal").option("--timezone <timezone>").requiredOption("--role-id <roleId>").option("--self-description <text>").option("--typical-task <text>", "Repeat for each typical task", collectValues, []).option("--ai-level <level>", "beginner|intermediate|advanced", parseAiLevel).option("--program-goal <text>").requiredOption("--persona <persona>", "support|efficiency", parsePersona).option("--response-length <length>", "short|balanced|detailed", parseResponseLength).action(async (o: { name?: string; assistantName?: string; addressForm?: "informal" | "formal"; timezone?: string; roleId: string; selfDescription?: string; typicalTask: string[]; aiLevel?: "beginner" | "intermediate" | "advanced"; programGoal?: string; persona: "support" | "efficiency"; responseLength?: "short" | "balanced" | "detailed" }) => { stdout.push(JSON.stringify(await employeeClient.completeOnboarding({ preferredName: o.name, assistantName: o.assistantName, addressForm: o.addressForm, timezone: o.timezone, roleId: o.roleId, selfDescription: o.selfDescription, typicalTasks: o.typicalTask.length ? o.typicalTask : undefined, aiLevel: o.aiLevel, programGoal: o.programGoal, persona: o.persona, responseLength: o.responseLength }))); }));
  employee.addCommand(new Command("profile").action(async () => { stdout.push(JSON.stringify(await employeeClient.getProfile())); }));
  employee.addCommand(new Command("context").action(async () => { stdout.push(JSON.stringify(await employeeClient.getPersonalContext())); }));

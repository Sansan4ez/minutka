...
export async function runMinutkaCli(client: EmployeeMinutkaClient | AdminMinutkaClient, argv: string[]): Promise<CliResult> {
  ...
  employee.addCommand(new Command("complete-onboarding").requiredOption("--role <role>").option("--task <task>", "Typical task", collect, []).requiredOption("--persona <persona>", "support|efficiency", parsePersona).requiredOption("--ai-level <level>", "beginner|intermediate|advanced", parseAiLevel).option("--response-length <length>", "short|balanced|detailed", parseResponseLength).action(async (o: { role: string; task: string[]; persona: "support" | "efficiency"; aiLevel: "beginner" | "intermediate" | "advanced"; responseLength?: "short" | "balanced" | "detailed" }) => { stdout.push(JSON.stringify(await employeeClient.completeOnboarding({ role: o.role, typicalTasks: o.task, persona: o.persona, aiLevel: o.aiLevel, responseLength: o.responseLength }))); }));

...
export const assistantDiagnosticProcessIds = ["morning_planning", "midday_adjustment", "personal_context_review", "consent_and_privacy", "evening_reflection", "weekly_summary"] as const satisfies readonly AssistantProcessId[];
export type AssistantDiagnosticProcessId = (typeof assistantDiagnosticProcessIds)[number];

/**
 * Diagnostic processes a schedule may fire — two daily touches and the weekly
 * personal summary. Narrower than the diagnostic set: `consent_and_privacy`
 * emits evidence inside a conversation but is never scheduled, so it has no
 * scheduled prompt.
 */
export const assistantScheduledProcessIds = ["morning_planning", "evening_reflection", "weekly_summary"] as const satisfies readonly AssistantDiagnosticProcessId[];

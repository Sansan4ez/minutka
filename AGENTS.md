...
«Минутка» — Telegram-first сервис исследования рабочих процессов и проектирования автоматизации (мультитенантный: компания → учебная группа → сотрудник → должность). Рабочий **прототип**: сотрудник получает AI-помощника, исследовательская команда — псевдонимизированный corpus и full traces, компания — подготовленную карту автоматизации. Цель — лёгкая, простая, быстрая и потому надёжная архитектура. Построен на Mastra + TypeScript.
...
**Целевая архитектура:** [docs/architecture/rfc-minutka-research-corpus-and-reporting.md](docs/architecture/rfc-minutka-research-corpus-and-reporting.md) (канонические messages/activities, group-scoped `subject_key`, full traces, evidence/evaluation и граница client report) поверх унаследованного рантайма: [rfc-personal-assistant-architecture.md](docs/architecture/rfc-personal-assistant-architecture.md) + [rfc-agent-led-routing.md](docs/architecture/rfc-agent-led-routing.md). Переиспользуемый паттерн — [research-corpus-reporting-pattern.md](docs/architecture/research-corpus-reporting-pattern.md). Старый [tenancy/reporting RFC](docs/architecture/rfc-minutka-tenancy-and-reporting.md) superseded в части anonymized dual-write/≥5/retention, но сохраняет провенанс tenant-модели.
...
- **Триаж каждой находки одним вопросом:** нарушает красную линию (owner/company/group isolation; company delivery boundary — наружу только report artifact без corpus/traces/subject keys; запись и research reads только через typed use-cases; подтверждение внешних действий; сохранность durable conversation/activity corpus; возможность purge/sanitize/recompute; секреты вне corpus/traces/model/logs/git) или ломает пилотный сценарий? Да → задача в эпике. Нет → P3/P4 в post-pilot backlog, фазу не блокирует.
